...
class ImageFilterOperation: Operation {
    ...
    var filter: String = "CIPhotoEffectFade"
    ...
    func addFilter(input: UIImage) -> UIImage? {
        ...
        let currentFilter = CIFilter(name: self.filter)
    ...
    override func main() {
        ...
        if let image = dependencyImage?.outputImage {
            guard !isCancelled else { return }
            self.processedImage = self.addFilter(input: image)
        }
    }
}
